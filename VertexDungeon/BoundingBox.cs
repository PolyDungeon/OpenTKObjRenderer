...
namespace ObjRenderer
{
	public class BoundingBox
	{
		...
		public BoundingBox(Vector4 min, Vector4 max)
		{
			...
			Vector4 diag;
			Vector4.Subtract(max, min, out diag);
			...
			Vector4 temp;
			Vector4.Add(min, xd, out temp);
			vertices[1] = temp;

            Vector4.Add(min, yd, out temp);
			vertices[2] = temp;

            Vector4.Subtract(max, zd, out temp);
			vertices[3] = temp;

            Vector4.Add(min,  zd, out temp);
			vertices[4] = temp;

            Vector4.Subtract(max, yd, out temp);
			vertices[5] = temp;

            Vector4.Subtract(max, xd, out temp);
			vertices[6] = temp;

			vertices[7] = max;
			
			Vector4.Divide( diag, 2, out temp);
            Vector4.Add(min,  temp, out center);
		}
	}
}
